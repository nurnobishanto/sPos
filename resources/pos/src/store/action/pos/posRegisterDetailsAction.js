...
import { apiBaseURL, posProductActionType, posRegisterDetailsAction, posRegisterReportDetailsAction, toastType } from '../../../constants';
...
export const registerCashInHandAction = ( data, navigate ) => ( dispatch ) => {
    ...
    apiConfig.post( url, data )
        .then( ( response ) => {
            ...
            dispatch( addToast(
                { text: getFormattedMessage( "register.entry.added.successfully.message" ) } ) );
        } )
        .catch( ( response ) => {
            dispatch( addToast(
...
export const closeRegisterAction = ( data, navigate ) => ( dispatch ) => {
    ...
    apiConfig.post( url, data )
        .then( ( response ) => {
            ...
            dispatch( addToast(
                { text: getFormattedMessage( "register.closed.successfully.message" ) } ) );
...
export const getAllRegisterDetailsAction = () => async ( dispatch ) => {
    apiConfig.get( apiBaseURL.GET_REGISTER_DETAILS )
        .then( ( response ) => {
            dispatch( { type: posRegisterDetailsAction.GET_REGISTER_DETAILS, payload: response.data.data } )
        } )
        .catch( ( { response } ) => {
            dispatch( addToast(
                { text: response.data.message, type: toastType.ERROR } ) );
...
export const getAllRegisterReportDetailsAction = ( { query = "", filter = {} } ) => async ( dispatch ) => {
